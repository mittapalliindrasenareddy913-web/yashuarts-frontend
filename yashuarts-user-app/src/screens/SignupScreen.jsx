...
import { Mail, Lock, Loader2, User, Palette, CheckCircle2, AlertTriangle } from 'lucide-react';
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setSuccess(true);
        setToast({ message: 'Account created successfully! Welcome to YashuArts!', type: 'success' });
        setTimeout(() => {
          setLoading(false);
          navigate('/');
        }, 1500);
      ...
      <div className="w-full max-w-md space-y-4">
        <div className="bg-[#161616]/70 border border-white/5 backdrop-blur-md rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative overflow-hidden">
          ...
          <form onSubmit={handleSignupSubmit} className="space-y-5 relative z-10">
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Password (Min. 6 characters)</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500">
          ...
          <p className="mt-6 text-center text-xs text-slate-400 relative z-10">
            ...
            <Link to="/login" className="text-[#D4AF37] hover:text-[#F3E5AB] font-semibold transition-colors">
              Sign In
            </Link>
          </p>
        </div>
